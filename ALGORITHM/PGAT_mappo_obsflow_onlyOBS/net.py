...
class Net(nn.Module):
    ...
    def _act(self, obs=None, act=None, message_obs=None, message_adv=None, type_mask=None, test_mode=None, eval_mode=False, eval_actions=None, avail_act=None, agent_ids=None, eprsn=None):
        ...
        if self.use_obs_pro_uhmp:
            ...
            other = other.reshape(other.shape[0], other.shape[1], other.shape[-2]*other.shape[-1])  # [n_threads, n_agents, n_entity-1 * rawob_dim]
        ...
        # 策略网络部分
        logits = self.AT_policy_head(h_obs)

        # Critic网络部分
        value = self.CT_get_value(h_obs)
